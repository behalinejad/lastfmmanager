    ...
    return Scaffold(
      appBar: AppBar(
        ...
        iconTheme: IconThemeData(
          color: context.read<ThemeCubit>().state.themeMode == ThemeMode.light ? Colors.black54 : Colors.white, //change your color here
        ),
      ),
 ...
 Widget _buildSearchPageBody() {
   ...
   return Column(
     ...
     children: [
       ...
       BlocBuilder<ArtistInfoBloc,ArtistInfoState>( /// the part that need to rebuild by bloc state management
           builder: (context,state) {
             ...
             else if (state is ArtistInfoIsLoading){    /// showing Circular Progress indicator while bloc is on isLoading state
               if (!_isPagination)
                 return  Padding(
                   padding:  EdgeInsets.only(top: 10.h),
                   child: Center(
                     child: Row(
                       ...
                       children: [
                         ...
                         SizedBox(width: 10,),
                         Text(' Loading ...',style: AppTextStyles.screenWarningTextStyle,)
                       ],
                     ),
                   ),
                 )  ;
             }else if (state is ArtistInfoIsLoaded){  /// while bloc is on isLoaded state and api has responded .
               _currentArtistInfo =  state.getArtistInfo;
               if (_isPagination){
                 _currentResults.addAll(_currentArtistInfo.results.artistMatches.artist) ;
                 refreshController.loadComplete();
                 _isPagination = false ;
               }
               else
                 _currentResults = _currentArtistInfo.results.artistMatches.artist;
               return _currentResults.length> 0 ? _buildCustomListView(_currentResults) : Padding(
                 padding:  EdgeInsets.only(top: 5.h),
                 child: Text(' OOPs , Nothing found ',style: AppTextStyles.screenWarningTextStyle,),
               );
             }
  ...
  Widget _buildCustomListView (List<Artist> currentResults) {
    try {
            ...
            child: Padding(
              ...
              child: SmartRefresher( // pagination  pull to refresh Widget
                ...
                onLoading: (){ // while the user reaches the end of the listView
                  ...
                  _isPagination = true ;
                  _currentPage ++;
                  if (_currentPage * int.parse(_currentArtistInfo.results.openSearchItemsPerPage) <= int.parse(_currentArtistInfo.results.openSearchTotalResults)  ){
                    final artistInfoBloc = BlocProvider.of<ArtistInfoBloc>(context);
                ...
                child: ListView.separated(
                    itemBuilder: (context,index){
                      ...
                      return Padding(
                        ...
                        child: CustomListTile(artist: result,),
                      );
                    },
      ...
      return Text('OOPs something went wrong ',style: AppTextStyles.screenWarningTextStyle,);
    }
  }
